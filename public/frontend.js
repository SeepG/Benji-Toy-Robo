...
const showReport = ()=>{
	roboReport = getRoboReport();
	if(roboReport){
		document.getElementById('message').innerHTML = `x: ${roboReport.position.x} <br/> y: ${roboReport.position.y}<br/> direction: ${roboReport.direction}`;
	}
	else{
		const err = ['Please execute the place command first before executing Report'];
		showError(JSON.stringify(err));
	}
}
...
const placeRobo = (req)=>{
 ...
 document.getElementById(`${req.position.x}${req.position.y}`).innerHTML = roboIcon;
}
